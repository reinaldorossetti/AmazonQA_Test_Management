...
- Local report generation task: `apiAllureReport`
- Local HTML output:
...
- On `push` to `main`, the pipeline publishes test reports to branch `gh-pages` using `peaceiris/actions-gh-pages`.
- Published base directory on `gh-pages`: `tests-report/`.
